...
- **Pattern recognition across <!-- AUTO-LC-COUNT -->77<!-- /AUTO-LC-COUNT --> LeetCode solutions** — two pointers, slow & fast, sliding window, Kadane's, Dutch National Flag, Moore's Voting, XOR, monotonic stack, priority queue, binary search, frequency counting, overflow bounds check, digit reversal, prefix sum, stack-based deletion.
- **Discipline** — <!-- AUTO-TOTAL-COUNT -->178<!-- /AUTO-TOTAL-COUNT --> solutions committed, CI-enforced strict-warnings compilation (`-Wall -Wextra -Wpedantic`) on every push.
...
**LeetCode by difficulty** (77 solved on [LeetCode profile](https://leetcode.com/u/madhan-x))
...
**Easy (7)**
- `backspace-string-compare.cpp` · `baseball-game-stack.cpp` · `implement-stack-using-queues.cpp` · `make-the-string-great.cpp` · `next-greater-element-i-monotonic-stack.cpp` · `remove-all-adjacent-duplicates-in-string.cpp` · `valid-parentheses-stack.cpp`

**Medium (2)**
- `removing-stars-from-a-string-stack.cpp` · `validate-stack-sequences.cpp`
