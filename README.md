...
- **Pattern recognition across <!-- AUTO-LC-COUNT -->109<!-- /AUTO-LC-COUNT --> LeetCode solutions** — two pointers, slow & fast, sliding window, Kadane's, Dutch National Flag, Moore's Voting, XOR, monotonic stack, priority queue, binary search, frequency counting, overflow bounds check, digit reversal, prefix sum, stack-based deletion.
- **Discipline & process** — <!-- AUTO-TOTAL-COUNT -->210<!-- /AUTO-TOTAL-COUNT --> solutions committed, CI-enforced strict-warnings compilation (`-Wall -Wextra -Wpedantic`) on every push, and stats auto-refreshed on every commit.
...
**LeetCode by difficulty** (109 solved on [LeetCode profile](https://leetcode.com/u/madhan-x))
...
**Medium (2)**
- `powx-n-binary-exponentiation.cpp` · `reverse-integer-overflowcheck.cpp`
...
**Easy (9)**
- `faulty-keyboard.cpp` · `find-first-occurrence-stl.cpp` · `find-the-index-of-the-first-occurrence-in-a-string-stl.cpp` · `first-unique-character-in-a-string-hash-map.cpp` · `isomorphic-strings-two-way-hash-map.cpp` · `length-of-last-word.cpp` · `reverse-string-two-pointers.cpp` · `reverse-vowels-of-a-string.cpp` · `valid-anagram-hashmap.cpp`
